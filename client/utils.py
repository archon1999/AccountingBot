...
def to_local_time(utc_dt: datetime.datetime, tzinfo):
    return (utc_dt + tzinfo.utcoffset()).replace(tzinfo=tzinfo)


def check_reservation_time(region: Region, dt: datetime.datetime):
    time = datetime.time(dt.hour, dt.minute)
    if time < region.working_time_from or time >= region.working_time_to:
        return False

    reservations = region.reservations(manager='actual').all()
    range_dt = (dt - timezone.timedelta(minutes=region.period-1),
                dt + timezone.timedelta(minutes=region.period-1))
    return not reservations.filter(datetime__range=range_dt).exists()


def get_next_unoccupied_time(region: Region, dt: datetime.datetime):
    while not check_reservation_time(region, dt):
        dt += timezone.timedelta(minutes=5)

    return dt


def get_datetime_range_for_day(dt: datetime.datetime):
    dt = dt.replace(hour=0, minute=0, second=0)
    date1 = dt
    date2 = (dt + timezone.timedelta(days=1))
    return (date1, date2)


def check_user_reservation_limit(user):
